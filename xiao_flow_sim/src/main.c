...
// Definición del UUID y establecimiento de este como constante. 
...
{
	ARG_UNUSED(attr); // Para evitar que el compilador llore

	notify_enabled = (value == BT_GATT_CCC_NOTIFY);
	LOG_INF("Notificaciones %s", notify_enabled ? "habilitadas" : "deshabilitadas");
}

// Macro que construye la tabla de atributos del servicio GATT para no tener que llamar al register
BT_GATT_SERVICE_DEFINE(flow_svc,
	BT_GATT_PRIMARY_SERVICE(&flow_svc_uuid), // Servicio primario 
	BT_GATT_CHARACTERISTIC(&flow_data_uuid.uuid, // Declaración y el valor del dato.
			       BT_GATT_CHRC_NOTIFY, BT_GATT_PERM_NONE, // Propiedad: notificar. Nada de acceso ni lectura
			       NULL, NULL, NULL),
	BT_GATT_CCC(data_ccc_changed, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE), 
);

// Contenido del paquete que se transmite
